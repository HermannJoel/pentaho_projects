...
ECHO *** Move to CONF_DIR : "%ROOT_DIR%\sales_dwh_ci-configuration\%PENTAHO_ENV%\env.pdi-server-windows.conf" ***

REM Load Environment Configuration
FOR /F "usebackq delims=" %%x IN ("%ROOT_DIR%\sales_dwh_ci-configuration\%PENTAHO_ENV%\env.pdi-server-windows.conf") DO (SET %%x)
